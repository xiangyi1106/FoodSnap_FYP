...
export default function TagPeople({
  ...
  const handleDelete = (usernameToDelete) => {
    setSelectedNames((prevSelectedNames) =>
      prevSelectedNames.filter((user) => user.username !== usernameToDelete)
    );
  };
  ...
  return (
    <div style={{ height: "400px" }}>
      <div
        className="create_post_popup_header"
        style={{ justifyContent: "center" }}
      >
        <div>
          <CIcon
            icon={cilArrowLeft}
            className="icon_size_22 icon_button exit_icon_left"
            onClick={() => setPage(0)}
          />
        </div>
        <span
          className="create_post_popup_header_title"
      ...
      <div className="tag_container">
        <TagNames selectedNames={selectedNames} onDelete={handleDelete} />
      </div>
      {isLoading && (
        <div
          className="middle"
          style={{ height: selectedNames.length > 0 ? "200px" : "260px" }}
        >
          {" "}
        ...
        >
          <List component="nav" aria-label="main mailbox folders">
            {filteredUsers.map((user) => (
              <div key={user._id}>
                <ListItem button onClick={() => handleNameClick(user)}>
                  <ListItemAvatar>
